...
public class CareerAnalysisController {
    ...
    public CareerAnalysisResponse analyzeCareer(
        ...
        if (dynamicRoadmap != null) {
            roadmap.setMonth1(dynamicRoadmap.size() > 0 ? dynamicRoadmap.get(0) : "Review Basics");
            roadmap.setMonth2(dynamicRoadmap.size() > 1 ? dynamicRoadmap.get(1) : "Intermediate Concepts");
            roadmap.setMonth3(dynamicRoadmap.size() > 2 ? dynamicRoadmap.get(2) : "Advanced Implementation");
            roadmap.setMonth4(dynamicRoadmap.size() > 3 ? dynamicRoadmap.get(3) : "Build Projects");
            roadmap.setMonth5(dynamicRoadmap.size() > 4 ? dynamicRoadmap.get(4) : "Cloud & Deployment");
            roadmap.setMonth6(dynamicRoadmap.size() > 5 ? dynamicRoadmap.get(5) : "Interview Preparation");
        }
